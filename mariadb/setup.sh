...
# Create AzuraCast DB
mariadb -e "create database $set_azuracast_database character set utf8mb4 collate utf8mb4_bin;"
mariadb -e "create user $set_azuracast_username@localhost identified by '$set_azuracast_password';"
mariadb -e "grant all privileges on $set_azuracast_database.* to $set_azuracast_username@localhost;"
...
if [ "$azuracast_git_version" = "stable" ] || [ "$azuracast_git_version" = "rolling" ]; then
    ...
    mariadb -sfu root <"mariadb/config/mysql_secure_installation.sql"
else
